...
#define array_gamma(a, s)  [Math.pow(a[0],1/s), Math.pow(a[1], 1/s), Math.pow(a[2], 1/s)]

#define array_dot(a, b)  (a[0]*b[0] + a[1]*b[1] + a[2]*b[2])
/*
...
#define array_clamped(a)  [(a)[0]<0?0:((a)[0]>1?1:(a)[0]), (a)[1]<0?0:((a)[1]>1?1:(a)[1]), (a)[2]<0?0:((a)[2]>1?1:(a)[2])]
...
#define array_normalized(a)  [a[0]/Math.sqrt(a[0]*a[0] + a[1]*a[1] + a[2]*a[2]), a[1]/Math.sqrt(a[0]*a[0] + a[1]*a[1] + a[2]*a[2]), a[2]/Math.sqrt(a[0]*a[0] + a[1]*a[1] + a[2]*a[2])]
...
#define array_scaled(a, s)  [a[0]*s, a[1]*s, a[2]*s]
/*
   ...
   */
#define array_plus(a, b)   [(a)[0] + (b)[0], (a)[1] + (b)[1], (a)[2] + (b)[2]]
...
#define array_mult(a, b) [a[0]*b[0], a[1]*b[1], a[2]*b[2]]
/*
...
#define array_minus(a, b) [a[0] - b[0], a[1]-b[1], a[2]-b[2]]
...
#define filter_array(a, b)  array_mult(a, b)
...
#define reverse(a)  array_scaled(a, -1)
...
function intersect_sphere(sindex, ray) {
	var op  = array_minus(spheres_cnt[sindex], ray[0]);
	var b   = array_dot(op, ray[1]);
	//var det = b*b - op.dot(op) + this.radius*this.radius;
	var det = b*b - array_dot(op, op) + spheres_rad[sindex]*spheres_rad[sindex];
	if(det < 0) {
		return 0;
	}
	var t;
...
function radiance(ray_, depth_) {
	...
	while(1) {
		...
		var x = array_plus(r[0], array_scaled(r[1], contact[1]));
		var n = array_normalized(array_minus(x, sphere[1]));
		var nl = array_dot(n, r[1]) < 0 ? n : array_scaled(n, -1);
		var f = sphere[2];
		var p = Math.max(sphere[2][0], sphere[2][1], sphere[2][2]);
		cl = array_plus(cl, array_mult(cf, sphere[3]));
		if(++depth > 5) if(get_random() < p) f = array_scaled(f, 1/p); else return cl;
		cf = array_mult(cf, f);
		...
		if(sphere[4] === 1) {
			...
			u = cross_product(u,w);
			u = array_normalized(u);
			var v = cross_product(w, u);
			var d = array_normalized(array_plus(array_plus(array_scaled(u, Math.cos(r1)*r2s), array_scaled(v, Math.sin(r1)*r2s)) , array_scaled(w, Math.sqrt(1-r2))));
			...
			r = new Array(x, array_minus(r[1], array_scaled(n, 2*array_dot(n, r[1]))), 0);
			continue;
		}
		//var reflRay = new Array(x, array_minus(r[1], array_mult(n, (array_scaled(array_dot(n, r[1]), 2)))), 0);
		var reflRay = new Array(x, array_minus(r[1], array_scaled(n, 2*array_dot(n, r[1]))), 0);
		var into = array_dot(n, nl) > 0;
		var nc = 1; var nt = 1.5; var nnt = into ? nc/nt:nt/nc;
		var ddn = array_dot(r[1], nl); var cos2t;
		...
		var a = nt-nc; var b = nt+nc; var R0 = a*a/(b*b); var c = 1-(into?-ddn:array_dot(tdir,n));
...
function combine_kernel2(index, samps, cx, cy, cam, c, canvasdim) {
	...
	for(var s = 0; s < samps; s++) {
		var d = array_plus(array_scaled(cx,x/w-0.5), array_scaled(cy,y/h-0.5));
		d = array_normalized(array_plus(d, cam[1]));
		var co = array_plus(cam[0], array_scaled(d, 140));
	...
	return array_scaled(array_clamped(color), 0.25);
}
...
function combine_kernel(index, samps, Xi, cx, cy, cam, c, canvasdim) {
	...
	var d = array_plus(array_plus(array_scaled(cx, scale1), array_scaled(cy, scale2)), cam[1]);
	var newRay = new Array(array_plus(cam[0], array_scaled(d, 140)), array_normalized(d), 0);


	var rad_result = radiance(newRay, 0, Xi);

	rad_result = array_scaled(rad_result, 1/samps);
	r = array_plus(r, rad_result);
	//}
	//this[i] = array_plus(this.get(i), array_scaled(array_clamped(r), 0.25));
	c[i] = array_plus(c[i], array_scaled(array_clamped(r), 0.25));
	//}
	//}
}
	...
	for(var sy = 0; sy<2; sy++) {
		for(var sx = 0; sx < 2; sx=sx+1) {
			...
			for(var s = 1; s < samps; s = s+1) {
					...
					if(!break_flag1) {
						...
						if(!break_flag2) {
							...
							if(rfl_sphere == 1) {
								...
								u = cross_product(u,wt);
								u = array_normalized(u);
								var v = cross_product(wt, u);
								...
								var d_tmp1 = array_scaled(u, Math.cos(r1)*r2s);
								var d_tmp2 = array_scaled(v, Math.sin(r1*r2s));
								var d_tmp3 = array_scaled(wt, Math.sqrt(1-r2));
								var d_tmp4 = array_plus(d_tmp1, d_tmp2);
								var d_tmp5 = array_plus(d_tmp4, d_tmp3);
								var d = array_normalized(d_tmp5);
								...
								var r_tmp1 = 2*array_dot(r_tmp0, r_direction);
								var r_tmp2 = array_scaled(r_tmp0, r_tmp1);
							...
							if(!continue_flag3) {
								...
								var reflRay_0 = x;
								var reflRay_1 = array_minus(r_direction, array_scaled(n_tmp0, 2*array_dot(n_tmp0, r_direction)));

								//var into = array_dot(n, nl) > 0;
								var into = array_dot(n_tmp0, nl_tmp0) > 0;
								var nc = 1; var nt = 1.5; var nnt = into ? nc/nt:nt/nc;
								var ddn = array_dot(r_direction, nl_tmp0); var cos2t;
								if((cos2t=1-nnt*nnt*(1-ddn*ddn)) < 0) {
									//r =reflRay;
									//r_origin = reflRay[0];
								...
								if(!continue_flag4) {
									...
									var tdir_tmp1 = array_scaled(n_tmp0, (into?1:-1)*(ddn*nnt+Math.sqrt(cos2t)));
									var tdir_tmp2 = array_scaled(r_direction, nnt);
									var tdir_tmp3 = array_minus(tdir_tmp2, tdir_tmp1);
									var tdir = array_normalized(tdir_tmp3);

									var a = nt-nc; var b = nt+nc; var R0 = a*a/(b*b); var c = 1-(into?-ddn:array_dot(tdir,n_tmp0));
									var Re = R0+(1-R0)*c*c*c*c*c; var Tr = 1-Re; var P = 0.25+0.5*Re; var RP = Re/P; var TP = Tr/(1-P);
									//if(Math.random() < P) {
									if(rand_val < P) {
										//cf = array_scaled(cf, RP);
				...
				rad_result = array_scaled(rad_result, 1/s);
...
function render() {
	...
	var camera_direction = array_normalized(camera_direction_raw);
	...
	var cy_tmp1 = cross_product(cx, cam[1]);
	var cy_tmp2 = array_normalized(cy_tmp1);
	var cy = array_scaled(cy_tmp2, 0.5135);
